...
MinhaMatriz_pt Matriz_Constroi(MinhaMatriz_pt me, unsigned int * tamanho, double * valores){
    ...
    static struct NumeroVtbl numVtbl = {  // pq não colocar o const?????? 
    ...
    //Tabela da matriz
    static struct MinhaMatriz_Interface_st interface = { // pq não colocar o const???? 
    ...
    return ptMat; // n seria returne me?, botar mensagem de erro na alocação? 
}
